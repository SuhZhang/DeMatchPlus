...
class LayerBlock(nn.Module):
    ...
    def forward(self, xs, d, feat_piece, logits=None, loss=False, cluster_score=None, decluster_score=None, topk_index_list=[], last_layer=False):
        ...
        # else:
        #     d = self.local_embed(d.unsqueeze(-1)).squeeze(-1)

        for layer, name in zip(self.cluster_layers, self.layer_names):
            if name == 'cluster':
                feat_piece, cluster_score, decluster_score = layer(feat_piece, d, mask, local=True) # BCP, BPN, BNP
            elif name == 'context':
                feat_piece = layer(feat_piece, feat_piece) # BCP
            elif name == 'decluster':
                d = layer(d, feat_piece) # BCN
            else:
                raise KeyError
        ...
        if loss:
            ...
            e_hat = None
        return d, logits, e_hat, cluster_score, decluster_score, topk_index_new #, cluster_list_index
    ...
    def forward(self, data, training=False):
        ...
        for i in range(self.layer_num):
            if i<self.layer_num-1:
                ...
                d, logits, e_hat, _, _, _ = self.layer_blocks[i](data['xs'], d, feat_piece, logits=logits, loss=True, cluster_score=cluster_score, decluster_score=decluster_score, topk_index_list=topk_index_list, last_layer=True) # BCN
                res_logits.append(logits), res_e_hat.append(e_hat)
